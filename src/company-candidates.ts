...
import type { Article, CandidateCompany, CandidateCompanyRegistry, TechnicalRoute } from "./types.js";
...
function officialEvidence(name: string, link: string): boolean {
...
function score(company: CandidateCompany): number {
  ...
  const official = company.officialUrl ? 60 : 0;
...
function status(company: CandidateCompany): CandidateCompany["status"] {
  if (company.officialUrl && company.verificationScore >= 70) return "已交叉核验";
...
export function updateCandidateCompanies(existing: CandidateCompanyRegistry | undefined, articles: Article[], now = new Date()): CandidateCompanyRegistry {
  ...
  for (const article of articles.filter((item) => item.kind === "投融资" && FUNDING.test(`${item.title} ${item.titleZh ?? ""}`) && TECH.test(`${item.title} ${item.titleZh ?? ""} ${item.excerpt}`))) {
    const name = entity(article); if (!name) continue;
    const key = name.toLowerCase();
    let company = companies.find((item) => item.aliases.some((alias) => alias.toLowerCase() === key));
    if (!company) {
      company = { id: `candidate-${createHash("sha256").update(key).digest("hex").slice(0, 12)}`, name, aliases: [name], status: "候选", verificationScore: 0, routes: [], firstSeenAt: now.toISOString(), lastSeenAt: now.toISOString(), evidence: [], openQuestions: [] };
    ...
    if (!company.officialUrl && officialEvidence(company.name, article.link)) company.officialUrl = new URL(article.link).origin;
    company.lastSeenAt = now.toISOString();
    company.verificationScore = score(company);
    company.status = status(company);
    company.openQuestions = company.status === "已交叉核验" ? ["待人工确认官网主营方向与公司简介后，才写入公开公司地图。"] : ["需要公司官网/投资方公告，或第二个独立媒体来源确认融资事实与主体。"];
  }
