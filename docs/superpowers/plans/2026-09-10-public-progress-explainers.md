...
- [ ] **Step 5: Verify two complete isolated fixture runs.** Use `mktemp -d /tmp/physical-ai-explainers.XXXXXX` and CLI `pnpm start -- --fixture --output-root "$fixture_dir"`; compare IDs/revisions and README order/content across both runs. Validate each output using exported `validateRelease(root)` from `src/validate-release.ts` (its CLI does not accept a root argument). If fixture has zero eligible reviewed content, test zero honestly and cover nonempty two-run behavior in injected full-generation integration test. Never hardcode fixture success in production validators.
