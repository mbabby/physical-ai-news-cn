...
test("accepts a complete versioned thesis and rejects missing falsifiers", () => {
  ...
  const { verifiedSensitiveFields: _verifiedSensitiveFields, ...legacy } = thesis;
  assert.equal(validateCompanyThesisShape(legacy), false);
  assert.equal(validateCompanyThesisShape({ ...thesis, verifiedSensitiveFields: ["secret"] }), false);
});
