...
test("README has shareable core entry points and a consistent evidence promise", async () => {
  ...
  assert.match(readme, /<summary>公司进展<\/summary>/);
  assert.match(readme, /<summary>研究论文<\/summary>/);
  assert.match(readme, /<summary>技术背景<\/summary>/);
  assert.match(readme, /AI 研究判断不属于事实/);
  assert.match(readme, /English overview/);
  assert.match(readme, /PROJECT_STATUS_START/);
  assert.match(readme, /运行记录/);
  assert.match(readme, /历史周报/);
  ...
  assert.ok(watchlistStart > readme.indexOf("<summary>公司进展</summary>"));
