...
首页与 README 的「近期解读」共同读取 `ProgressExplainersArtifact`，每轮最多三篇；前端不得重新排序、评分、编写解读或用旧 dashboard 新闻补位。每篇保留两条事实、变化、明确标注的「解读」、局限和可展开的原始证据。比较只在任务与条件可比且有证据时出现；演示、仿真、作者报告不升级为真实部署或独立复现。

内容先通过结构与证据绑定检查，再进行独立的模型语义复核。复核是辅助检查，不保证绝对正确，不替代读者查阅原始证据。解读不构成新增事实、投资建议或行动催促。

「有新解读」「本轮无新内容」「本轮受限」「暂不可用」是不同状态。检查时间、内容更新时间、事件日期、披露日期和实质变化日期分别保留；未知日期不补写运行时间。无新内容不得改写旧卡片日期制造新鲜感，历史内容显式标注。来源撤回、冲突或依赖变化时优先撤下失效内容，模型失败不能强留失效卡片。工件与 README 在同一事务内发布，候选层永远不是补位来源。

## 资料库旧版研究产品排序

以下排序和旧版「首页」术语仅指资料库保留的产业/资本/论文研究产品，不控制现在的近期解读。
...
Core 30 是季度固定的研究资源配置，不是公司排名、投资评分、市场份额或法律国籍判断。覆盖主体、30 秒 Brief、12 个月时间线、90 天事件/披露窗口、季度变更、README 与独立 Feed 均读取同一轮通过校验的公开工件；身份覆盖数与完整 Brief 数分别报告，配置中的 30 行不能冒充 30 个已核验主体或 30 份完整研究。

资本、产品、客户、部署与验证阶段只显示当前工件中逐字段 `verified` 且绑定直接证据的值；验证阶段不得从旧事件类型或营销表述推断。资本为 unknown 时统一表述为“现有证据不足”，不代表未融资。平台和实验室以研究资源、公开产出与验证证据描述，不要求或暗示创业公司融资。覆盖地区仅表示编辑研究配额，不替代经营地区、注册地、母公司或集团关系证据。

Core 30 Feed 只收录当前窗口内具备明确事件日期或明确披露日期、已核验事实 ID 与可追溯实质变化日期的条目。首次回填或本轮生成时间不得把旧事实伪装为新发生事项；未知实质变化日期不补写生成时间。当前工件、公开历史、README 与 Feed 的时钟、版本、事实 ID 或字节镜像任一不一致时，整组发布失败并保留上一版。
